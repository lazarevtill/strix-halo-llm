...
$known = @(
    ...
    @{ match = 'Ornith-1.5-35B-Q6_K';                label = 'ornith15';          spec = @('spec-type = draft-mtp','spec-draft-n-max = 3'); mmproj = 'mmproj-Ornith-1.5-35B-BF16.gguf' }                  # Ornith-1.5-35B-A3B: successor to ornith-1.0 (same org -- deepreinforce-ai 307-redirects to ornith-ai). 36B/A3B, arch qwen35moe (standard attention, NOT an SSM hybrid -> no #27805 class), 262144 native, MIT. VISION via first-party mmproj. THINKING model (<think> -> reasoning_content) => needs ample max-tokens or `content` returns EMPTY. 256 experts, so the b11046 >256-expert fix (#28501) does NOT apply to it -- that win is coder-only. HAS an MTP head (nextn_predict_layers=1) so draft-mtp is available: n=3 is llama.cpp's default and the measured peak on qwen38, but it is UNMEASURED on this model -- A/B with bench-spec.ps1 -NMax 1,2,3,4 before trusting. Sampling temp 0.6/top-p 0.95/top-k 20 == the $common defaults already, so no override needed.
